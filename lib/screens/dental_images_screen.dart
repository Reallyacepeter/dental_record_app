...
//   @override
//   Widget build(BuildContext context) {
//     final pvd = context.watch<DentalDataProvider>();
//     otherRadiographsController.text = pvd.otherRadiographs;
//
//     final metas = _filtered(pvd);
//     final folders = _allFolders(pvd);
//     final tags = _allTags(pvd);
//
//     return Scaffold(
//       appBar: const CommonAppBar(
//         title: "615 : Dental Images",
...
//
//   // ===== 상단 라디오/텍스트 입력 (원래 폼) =====
//   Widget _topChecks(DentalDataProvider pvd) {
//     return Padding(
//       padding: const EdgeInsets.fromLTRB(16, 12, 16, 4),
//       child: Column(
//         crossAxisAlignment: CrossAxisAlignment.stretch,
//         children: [
//           _checkRow("PA", pvd.paDigital, pvd.paNonDigital, (v) => pvd.setDentalImages(paD: v), (v) => pvd.setDentalImages(paND: v)),
//           _checkRow("BW", pvd.bwDigital, pvd.bwNonDigital, (v) => pvd.setDentalImages(bwD: v), (v) => pvd.setDentalImages(bwND: v)),
//           _checkRow("OPG", pvd.opgDigital, pvd.opgNonDigital, (v) => pvd.setDentalImages(opgD: v), (v) => pvd.setDentalImages(opgND: v)),
//           _checkRow("CT", pvd.ctDigital, pvd.ctNonDigital, (v) => pvd.setDentalImages(ctD: v), (v) => pvd.setDentalImages(ctND: v)),
//           const SizedBox(height: 4),
//           const Align(alignment: Alignment.centerLeft, child: Text("Other Radiographs", style: TextStyle(fontWeight: FontWeight.bold))),
//           TextField(
//             controller: otherRadiographsController,
//             onChanged: (v) => pvd.setDentalImages(otherRadio: v),
//             decoration: const InputDecoration(hintText: "Specify other radiographs"),
//           ),
//           _checkRow("Other", pvd.otherDigital, pvd.otherNonDigital, (v) => pvd.setDentalImages(otherD: v), (v) => pvd.setDentalImages(otherND: v)),
//           _checkRow("Photographs", pvd.photographsDigital, pvd.photographsNonDigital, (v) => pvd.setDentalImages(photoD: v), (v) => pvd.setDentalImages(photoND: v)),
//         ],
//       ),
//     );
//   }
...
//
//   // ===== 툴바(검색/필터/소트/그리드-리스트/멀선/일괄삭제) =====
//   Widget _toolbar(int visibleCount, List<String> folders, List<String> tags) {
//     return Padding(
//       padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
//       child: Wrap(
//         spacing: 8,
...
//             items: [
//               const DropdownMenuItem<String?>(value: null, child: Text('All Folders')),
//               ...folders.map((f) => DropdownMenuItem<String?>(value: f, child: Text(f))),
//             ],
//             onChanged: (v) => setState(() => folderFilter = v),
//           ),
//           // 태그 필터 (null 허용)
...
//           // 멀티선택/삭제
//           FilterChip(
//             label: Text(multiselect ? '선택중 (${selected.length})' : '멀티선택'),
//             selected: multiselect,
//             onSelected: (v) {
//               setState(() {
//                 multiselect = v;
...
//                 }
//               }),
//               child: Text(selected.length == visibleCount ? '전체 해제' : '전체 선택'),
//             ),
//           ],
//         ],
//       ),
...
//
//     if (m.kind == MediaKind.video) {
//       // 웹/모바일 공통: 썸네일이 없어도 안전한 폴백
//       return ClipRRect(
//         borderRadius: br,
//         child: Stack(
//           children: [
//             Positioned.fill(child: Container(width: w, height: h, color: Colors.black12)),
//             const Positioned.fill(
//               child: Center(child: Icon(Icons.play_circle, size: 42, color: Colors.white70)),
//             ),
//           ],
...
//
//   // ===== 데이터 정렬/필터/검색 =====
//   List<UploadedFileMeta> _filtered(DentalDataProvider pvd) {
//     var metas = pvd.uploadedFilesMeta.values.toList();
//
//     // 레거시 문자열 리스트와 동기 보정(혹시 남아있다면)
//     for (final u in pvd.uploadedFiles) {
//       if (!pvd.uploadedFilesMeta.containsKey(u)) {
//         metas.add(_metaFromUrl(u));
//       }
//     }
//
...
//   }
//
//   List<String> _allFolders(DentalDataProvider pvd) {
//     final s = <String>{};
//     for (final m in pvd.uploadedFilesMeta.values) {
//       if ((m.folder ?? '').isNotEmpty) s.add(m.folder!);
//     }
//     return s.toList()..sort();
//   }
//
//   List<String> _allTags(DentalDataProvider pvd) {
//     final s = <String>{};
//     for (final m in pvd.uploadedFilesMeta.values) {
//       s.addAll(m.tags);
//     }
...
//
//   // ===== 업로드/삭제/열기 =====
//   /// Storage 경로 prefix를 동적으로 생성합니다.
//   /// DentalDataProvider 쪽에 uid / recordId 필드명이 다르면 아래 접근자를 바꿔주세요.
//   String _uploadsPrefix(DentalDataProvider pvd) {
//     final uid = ((){
//       try { return (pvd as dynamic).uid as String?; } catch (_) { return null; }
//     })();
//     final rec = ((){
//       try { return (pvd as dynamic).currentRecordId as String?; } catch (_) { return null; }
//     })();
//     final u = (uid == null || uid.isEmpty) ? 'public' : uid;
//     final r = (rec == null || rec.isEmpty) ? 'unassigned' : rec;
//     return 'uploads/$u/$r';
//   }
//
//
...
//     if (result == null) return;
//
//     final pvd = context.read<DentalDataProvider>();
//     final urls = <String>[];
//     final prefix = _uploadsPrefix(pvd);
//     const maxUploadMB = 100; // 필요 시 조정
...
//         final bytes = f.bytes;
//         if (bytes == null) {
//           // 웹/안드 용량 이슈 등으로 bytes가 없으면 스킵
//           // (모바일 대용량 지원이 꼭 필요하면 별도 화면에서 chunk 업로드 구현 권장)
//           continue;
//         }
//         if (bytes.length > maxBytes) {
//           ScaffoldMessenger.of(context).showSnackBar(
//             SnackBar(content: Text('용량 초과: ${f.name} (>${maxUploadMB}MB)')),
//           );
//           continue;
//         }
//         final fileName = p.basename(f.name);
//         final ts = DateTime.now().millisecondsSinceEpoch;
//         final ref = FirebaseStorage.instance.ref().child('$prefix/${ts}_$fileName');
//         final metadata = SettableMetadata(contentType: _guessMime(fileName));
...
//         // 메타 자동 생성 + 저장
//         final meta = _metaFromUrl(url);
//         pvd.setUploadedFileMeta(meta); // 신규 저장
//       }
//       // 레거시 리스트에도 추가(겸용)
//       final updated = [...pvd.uploadedFiles, ...urls];
//       pvd.setDentalImages(uploads: updated);
//
//       ScaffoldMessenger.of(context).hideCurrentSnackBar();
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('파일이 업로드 되었습니다.')));
//     } catch (e) {
...
//       final ref = FirebaseStorage.instance.refFromURL(url);
//       await ref.delete();
//       pvd.removeUploadedFile(url);
//       pvd.removeUploadedFileMeta(url);
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('삭제 완료')));
//     } catch (e) {
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('삭제 실패: $e')));
//     }
...
//     ScaffoldMessenger.of(context).hideCurrentSnackBar();
//     ScaffoldMessenger.of(context).showSnackBar(
//       SnackBar(content: Text('삭제 완료: $success건, 실패: $fail건')),
//     );
//   }
//
//   Future<void> _openInApp(UploadedFileMeta m) async {
...
//                           tags: tags,
//                           description: descCtl.text.trim().isEmpty ? null : descCtl.text.trim(),
//                           takenAt: takenCtl.text.trim().isEmpty
//                               ? null
//                               : DateTime.tryParse('${takenCtl.text.trim()} 12:00:00'),
//                         );
//                         pvd.setUploadedFileMeta(upd);
//                         Navigator.pop(context);
//                       },
//                       icon: const Icon(Icons.save),
//                       label: const Text('저장'),
...
import 'dart:io' show File; // 모바일(안드)에서 putFile 사용
import 'package:flutter/material.dart';
import 'package:flutter/foundation.dart' show kIsWeb;
import 'package:file_picker/file_picker.dart';
import 'package:firebase_storage/firebase_storage.dart';
...
class _DentalImagesScreenState extends State<DentalImagesScreen> {
  ...
  bool _syncedOnce = false; // controller 초기 동기화 가드
  ...
  String _uploadsPrefix(DentalDataProvider pvd) {
    final uid = (() {
      try {
        return (pvd as dynamic).uid as String?;
      } catch (_) {
        return null;
      }
    })();
    final rec = (() {
      try {
        return (pvd as dynamic).currentRecordId as String?;
      } catch (_) {
        return null;
      }
    })();
  ...
  Future<void> _uploadFiles() async {
    ...
    try {
      for (final f in result.files) {
        ...
        if (kIsWeb) {
          final bytes = f.bytes;
          if (bytes == null) continue; // 웹인데 bytes가 없을 일은 드묾
          if (bytes.length > maxBytes) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('용량 초과: ${f.name} (> ${maxUploadMB}MB)')),
            );
          ...
          // 모바일/데스크탑: 파일 경로 스트리밍 업로드 (메모리 절약)
          final path = f.path;
          if (path == null) continue; // 보호
  ...
  Future<void> _deleteOne(String url, {bool silent = false}) async {
    final pvd = context.read<DentalDataProvider>();
    try {
      final ref = FirebaseStorage.instance.refFromURL(url);
      await ref.delete();
  ...
  Future<void> _deleteSelected() async {
    ...
    for (final url in toDel) {
      try {
        await _deleteOne(url, silent: true);
        success++;
      } catch (_) {
        fail++;
      }
  ...
  // ===== 메타데이터 편집 바텀시트 (StatefulBuilder로 분리) =====
  Future<void> _showMetaSheet(UploadedFileMeta meta) async {
    final pvd = context.read<DentalDataProvider>();
            ...
            return Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(meta.name, style: Theme.of(sheetCtx).textTheme.titleMedium),
                    const SizedBox(height: 12),
                    Wrap(spacing: 8, runSpacing: 8, children: [
                      DropdownButton<MediaKind>(
                        value: kind,
                        items: MediaKind.values
                            .map((k) => DropdownMenuItem(value: k, child: Text(k.name.toUpperCase())))
                            .toList(),
                        onChanged: (v) => setSheetState(() => kind = v ?? kind),
                      ),
                      SizedBox(
                        width: 220,
                        child: TextField(
